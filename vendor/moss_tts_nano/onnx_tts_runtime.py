...
# vox: torch/torchaudio removed (only used to load the reference clip upstream);
# see _load_reference_audio below. Output dir is a temp dir; vox never writes wavs.
APP_DIR = Path(__file__).resolve().parent
REPO_ROOT = APP_DIR
DEFAULT_OUTPUT_DIR = Path(tempfile.gettempdir()) / "vox-moss"
...
def ensure_browser_onnx_model_dir(model_dir: str | Path | None = None) -> Path:
    ...
    # vox: download into whatever directory was given (upstream only auto-downloaded
    # into its repo-local default).
...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        """vox: load a reference clip with soundfile -> float32 [1, channels, samples].

        Upstream resampled with torchaudio; vox stores voices at the codec's native
        rate when they are added, so a mismatch here is a caller error, not a job."""
        ...
        data, sample_rate = sf.read(str(path), dtype="float32", always_2d=True)   # [N, C]
        if int(sample_rate) != target_sample_rate:
            raise ValueError(f"reference audio must be {target_sample_rate} Hz, got {sample_rate}: {path}")
        waveform = np.ascontiguousarray(data.T, dtype=np.float32)                   # [C, N]
        ...
        return np.ascontiguousarray(waveform, dtype=np.float32)[None, :, :]
